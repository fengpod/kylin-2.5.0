...
public class SparkBatchCubingJobBuilder2 extends JobBuilderSupport {
    ...
    private static final String ENGINE_TYPE_CONF = "hfilejob.enginetype";
    private static final String ENGINE_TYPE_MR = "mr";
...
//        result.addTask(createFactDistinctColumnsSparkStep(jobId));
        ...
        //hfile这步优先使用override中配置的引擎类型
        LinkedHashMap<String, String> overrideKylinProps = seg.getCubeDesc().getOverrideKylinProps();
        if(overrideKylinProps.containsKey(ENGINE_TYPE_CONF) && overrideKylinProps.get(ENGINE_TYPE_CONF).equalsIgnoreCase(ENGINE_TYPE_MR)) {
            outputSide.addStepPhase3_MRBuildCube(result);
        }else{
