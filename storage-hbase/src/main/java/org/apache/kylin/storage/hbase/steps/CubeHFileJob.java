...
public class CubeHFileJob extends AbstractHadoopJob {
    ...
    public int run(String[] args) throws Exception {
        ...
        try {
            ...
            Configuration oriConfiguration = HBaseConnection.getCurrentHBaseConfiguration();

            Configuration configuration = new Configuration();
            HBaseConfiguration.merge(configuration, getConf());
            HBaseConfiguration.merge(configuration, oriConfiguration);

            //cube to hfile时 不需要访问hbase的hdfs
            FileSystem fs = HadoopUtil.getWorkingFileSystem(HadoopUtil.getCurrentConfiguration());
            configuration.set(FileSystem.FS_DEFAULT_NAME_KEY, fs.getUri().toString());
